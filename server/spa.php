...
function insert() {
  ...
  $sql = "insert into pet_phc_spa (customerid, customerid2, doctorid, note, time, utime, weight, image, treat) values($customer[id], $customer2[id], $userid, '$data->note', '" . time() . "', '" . time() . "', $data->weight, '". implode(', ', $data->image)."', $data->treat)";
  $id = $db->insertid($sql);
...
function getList() {
  ...
  $end = $time + 60 * 60 * 24 - 1;
  $sql = "select a.*, b.name, b.phone, c.name as user from pet_phc_spa a inner join pet_phc_customer b on a.customerid = b.id inner join pet_phc_users c on a.doctorid = c.userid where (time between $time and $end) and status < 3 order by utime desc";
  $spa = $db->all($sql);
