...
//   $sql = "select a.*, b.name, b.phone, b.address, c.hoten as user from pet_". PREFIX ."_sieuam a inner join pet_". PREFIX ."_customer b on a.customerid = b.id inner join pet_nhanvien c on a.userid = c.id where a.time between $start and $end order by id desc";
//   $list = $db->all($sql);
...
//   $time = time();
//   $sql = "insert into pet_". PREFIX ."_sieuam (userid, customerid, image, note, time) values($data->idnguoidung, $customerid, '$image', '$data->note', $time)";
//   $id = $db->insertid($sql);
...
//   $sql = "select a.*, b.name, b.phone, b.address, c.hoten as user from pet_". PREFIX ."_sieuam a inner join pet_". PREFIX ."_customer b on a.customerid = b.id inner join pet_nhanvien c on a.userid = c.id where a.id = $data->id";
//   $data = $db->fetch($sql);
